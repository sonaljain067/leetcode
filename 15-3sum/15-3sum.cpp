...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        //Way1: Brute force TC: O(n^3), SC:O(1) -> TLE
        ...
        // Way2.1: Hashmap TC:O(n^2), SC: O(n)
...
//                 j=mp.find(nums[j])->second; //update to avoid j duplicate's
//             }
//             i=mp.find(nums[i])->second; //update to avoid i duplicate's
//             }
//         }
//         return ans;

        // Way2.2: Hashmap TC:O(n^2), SC: O(n)
        sort(nums.begin(), nums.end()); int n = nums.size();
        if (n < 3 || n == 0 || nums[0] > 0) return {};
        unordered_map<int, int> mp; vector<vector<int>> ans;
        ...
        for (int i = 0; i < n - 2; i++){
            ...
            for (int j = i + 1; j < n - 1; j++){
                ...
                if (mp[-(nums[i] + nums[j])] > j) ans.push_back({nums[i], nums[j], -(nums[i] + nums[j])});
            }
        }
        return ans;
        
        // Way3: Two pointers/Vector TC: O(n^2), SC: O(1)
